...
export default function BottomNav() {
  const navRef = useRef(null) // ✅ sem <HTMLDivElement | null>
  const navigate = useNavigate()
  const location = useLocation()

  const phone = useMemo(() => '91980824554', [])
  const [activeOrdersCount, setActiveOrdersCount] = useState(0)

  const onlyDigits = (v) => String(v || '').replace(/\D/g, '')
  const isActiveStatus = (s) => !['concluido', 'cancelado'].includes(String(s))

  const hide =
    /^\/(produto|product)\/[^/]+$/.test(location.pathname) ||
  ...
  const loadActiveCount = async () => {
    try {
      const byPhone = await ApiService.request(`/orders/by-phone?phone=${phone}`)
      const orders = byPhone?.orders || []
      const count = orders.filter((o) => isActiveStatus(o.status)).length
  ...
  useEffect(() => {
    ...
    const myPhone = onlyDigits(phone)

    s.on('order:new', (order) => {
      if (onlyDigits(order?.cliente?.telefone) !== myPhone) return
      if (isActiveStatus(order?.status)) setActiveOrdersCount((prev) => prev + 1)
    })
  ...
  }, [cartItemsCount, activeOrdersCount])
  ...
  const navItems = [
    { path: '/', icon: Home, label: 'Início' },
    { path: '/cart', icon: ShoppingCart, label: 'Carrinho', badge: cartItemsCount },
    { path: '/order', icon: ReceiptText, label: 'Pedidos', badge: activeOrdersCount },
    { path: '/profile', icon: User, label: 'Perfil' }
  ]
